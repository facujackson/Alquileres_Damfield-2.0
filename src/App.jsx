...
function LoginScreen({users, onLogin}) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"radial-gradient(ellipse at 50% 15%,#3d3530 0%,#252525 55%,#1a1a1a 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{width:"100%",maxWidth:380}}>
        {/* Logo */}
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{width:56,height:56,borderRadius:16,background:"linear-gradient(135deg,#4a3f38 0%,#2b2b2b 100%)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 12px",padding:8,boxShadow:"0 8px 24px rgba(0,0,0,0.5)"}}><DamfieldLogo size={40}/></div>
          <div style={{fontSize:22,fontWeight:800,color:"#fff"}}>Damfield</div>
          <div style={{fontSize:12,color:"#7c7876",textTransform:"uppercase",letterSpacing:1.5,marginTop:2}}>Sistema de Alquileres</div>
        </div>

        <div style={{background:"linear-gradient(160deg,#363330 0%,#2b2b2b 100%)",border:"1px solid #454545",borderRadius:16,padding:28,boxShadow:"0 24px 64px rgba(0,0,0,0.6)"}}>
          ...
          <div style={{marginBottom:20}}>
            ...
            <div style={{position:"relative"}}>
              ...
              <button type="button" onClick={()=>setShowPwd(p=>!p)} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#7c7876",fontSize:16,padding:2}}>
                {showPwd?"🙈":"👁"}
              </button>
            </div>
          </div>
          ...
          <button onClick={attempt} disabled={loading||!username||!password} style={{
            ...
            background:(!username||!password)?"#454545":"#BA9F82",
            color:(!username||!password)?"#7c7876":"#2b2b2b",
          }}>{loading?"Ingresando…":"Ingresar"}</button>
          <button type="button" onClick={async()=>{
            await dbSet("users", DEFAULT_USERS);
            window.location.reload();
          }} style={{width:"100%",marginTop:10,padding:"8px",borderRadius:8,border:"1px solid #454545",background:"transparent",color:"#7c7876",cursor:"pointer",fontSize:11}}>
...
function BookingBottomSheet({ form, setForm, editing, onSave, onClose, onDelete,
  ...
  const inpMob = { width:'100%', background:'#2b2b2b', border:'1px solid #454545', borderRadius:10, padding:'12px 14px', color:'#E2E0E1', fontSize:14, boxSizing:'border-box' };
  const lblMob = { fontSize:10, color:'#7c7876', textTransform:'uppercase', letterSpacing:0.5, display:'block', marginBottom:4 };

  return (
    <>
      <div style={{position:'fixed',inset:0,background:'#000000a0',zIndex:100}} onClick={onClose}/>
      <div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:101,background:'#2b2b2b',borderRadius:'20px 20px 0 0',border:'1px solid #454545',borderBottom:'none',maxHeight:'92vh',overflowY:'auto',padding:'0 16px 32px'}}>
        {/* Handle */}
        <div style={{display:'flex',justifyContent:'center',padding:'12px 0 8px'}}>
          <div style={{width:36,height:4,background:'#454545',borderRadius:2}}/>
        </div>
        {/* Header */}
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
          <div style={{fontSize:16,fontWeight:800}}>{editing?'Editar alquiler':'Nuevo alquiler'}</div>
          <button onClick={onClose} style={{background:'none',border:'none',color:'#7c7876',fontSize:22,cursor:'pointer'}}>×</button>
        </div>
        {/* Progress bar */}
        {!form.isBloqueo&&(
          <div style={{display:'flex',gap:4,marginBottom:20}}>
            {steps.map((s,i)=>(
              <div key={i} onClick={()=>setStep(i)} style={{flex:1,height:4,borderRadius:2,cursor:'pointer',background:i<=step?'#BA9F82':'#454545',transition:'background 0.2s'}}/>
            ))}
          </div>
        )}
        {/* Tipo toggle */}
        <div style={{display:'flex',gap:6,marginBottom:16}}>
          {[{v:false,l:'📅 Alquiler'},{v:true,l:'🚫 Bloquear'}].map(({v,l})=>(
            <button key={String(v)} onClick={()=>{setForm(f=>({...f,isBloqueo:v}));setStep(0);}} style={{flex:1,padding:12,borderRadius:10,border:`2px solid ${form.isBloqueo===v?(v?'#c96b5f':'#BA9F82'):'#454545'}`,background:form.isBloqueo===v?(v?'#c96b5f18':'#BA9F8218'):'transparent',color:form.isBloqueo===v?(v?'#c96b5f':'#BA9F82'):'#7c7876',cursor:'pointer',fontWeight:700,fontSize:13}}>{l}</button>
          ))}
        </div>
        ...
        {(step===0||form.isBloqueo)&&(
          <div>
            ...
            <div style={{background:'#333333',borderRadius:10,padding:14,marginBottom:16,border:'1px solid #454545'}}>
              <div style={{fontSize:11,fontWeight:700,color:'#a7a3a0',marginBottom:10}}>🔁 Repetición semanal</div>
              <div style={{display:'flex',gap:6}}>
                {[['none','No repetir'],['count','N semanas'],['until','Hasta fecha']].map(([v,l])=>(
                  <button key={v} onClick={()=>setForm(f=>({...f,recurrence:v}))} style={{flex:1,padding:'8px 4px',borderRadius:8,border:'1px solid',fontSize:11,cursor:'pointer',borderColor:form.recurrence===v?'#BA9F82':'#454545',background:form.recurrence===v?'#BA9F8222':'transparent',color:form.recurrence===v?'#BA9F82':'#7c7876'}}>{l}</button>
                ))}
              </div>
              {form.recurrence==='count'&&<div style={{marginTop:10,display:'flex',alignItems:'center',gap:10}}><span style={{fontSize:12,color:'#7c7876'}}>Semanas:</span><input type="number" min={1} max={52} value={form.recurrenceCount} onChange={e=>setForm(f=>({...f,recurrenceCount:+e.target.value}))} style={{width:60,background:'#2b2b2b',border:'1px solid #454545',borderRadius:8,padding:'8px',color:'#E2E0E1',fontSize:14}}/></div>}
              {form.recurrence==='until'&&<input type="date" value={form.recurrenceUntil} onChange={e=>setForm(f=>({...f,recurrenceUntil:e.target.value}))} style={{marginTop:10,width:'100%',background:'#2b2b2b',border:'1px solid #454545',borderRadius:8,padding:'10px',color:'#E2E0E1',fontSize:14,boxSizing:'border-box'}}/>}
            </div>
            <button onClick={form.isBloqueo?onSave:()=>setStep(1)} disabled={form.isBloqueo&&!(form.bloqueoMotivo||'').trim()} style={{width:'100%',padding:14,borderRadius:12,border:'none',fontSize:15,fontWeight:700,cursor:'pointer',background:'#BA9F82',color:'#2b2b2b'}}>
        ...
        {!form.isBloqueo&&step===1&&(
          <div>
            <label style={{display:'flex',alignItems:'center',gap:10,marginBottom:14,cursor:'pointer',background:form.sinCargo?'#BA9F820d':'#333333',padding:'12px 14px',borderRadius:10,border:`1px solid ${form.sinCargo?'#BA9F8244':'#454545'}`}}>
              <input type="checkbox" checked={form.sinCargo} onChange={e=>setForm(f=>({...f,sinCargo:e.target.checked}))} style={{width:18,height:18,accentColor:'#BA9F82'}}/>
              <div><div style={{fontSize:14,fontWeight:700,color:form.sinCargo?'#BA9F82':'#a7a3a0'}}>🎁 Sin cargo</div><div style={{fontSize:11,color:'#7c7876'}}>Cortesía, uso interno…</div></div>
            </label>
            {form.sinCargo?(
              ...
              <>
                {form.clientName?(
                  <div style={{display:'flex',alignItems:'center',gap:8,background:'#333333',borderRadius:10,padding:'12px 14px',border:'1px solid #454545',marginBottom:14}}>
                    <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700}}>{form.clientName}</div>{form.clientOrg&&<div style={{fontSize:12,color:'#7c7876'}}>{form.clientOrg}</div>}</div>
                    <button onClick={()=>setForm(f=>({...f,clientId:'',clientName:'',clientPhone:'',clientEmail:'',clientOrg:''}))} style={{background:'none',border:'none',color:'#7c7876',cursor:'pointer',fontSize:20}}>×</button>
                  </div>
                ):(
                  <button onClick={()=>setClientPicker(true)} style={{width:'100%',padding:'14px',borderRadius:10,border:'2px dashed #454545',background:'transparent',color:'#7c7876',cursor:'pointer',fontSize:14,marginBottom:14}}>👥 Elegir cliente existente…</button>
                )}
                {!form.clientId&&(
          ...
          <div>
            {!form.sinCargo&&(
              <>
                ...
                {(form.pagos||[]).length>0&&(
                  <div style={{background:'#333333',borderRadius:10,padding:12,marginBottom:14}}>
                    <div style={{fontSize:10,color:'#7c7876',textTransform:'uppercase',letterSpacing:1,marginBottom:8}}>Pagos registrados</div>
                    {(form.pagos||[]).map(p=>(
                      <div key={p.id} style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                        ...
                        <span style={{fontSize:11,color:'#7c7876'}}>{{efectivo:'💵',transferencia:'🏦',debito:'💳',credito:'💳',cuentacorriente:'📒'}[p.forma]||''} {p.forma}</span>
                      </div>
                    ))}
                    <div style={{borderTop:'1px solid #454545',marginTop:6,paddingTop:6,display:'flex',justifyContent:'space-between'}}>
                      <span style={{fontSize:11,color:'#7c7876'}}>Restante</span>
            ...
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>setStep(1)} style={{flex:1,padding:14,borderRadius:12,border:'1px solid #454545',background:'transparent',color:'#a7a3a0',cursor:'pointer',fontSize:14,fontWeight:700}}>← Atrás</button>
              <button onClick={onSave} disabled={!form.date||(!form.isBloqueo&&!form.sinCargo&&!form.clientName)} style={{flex:2,padding:14,borderRadius:12,border:'none',fontSize:15,fontWeight:700,cursor:'pointer',background:'#BA9F82',color:'#2b2b2b'}}>
...
export default function App() {
  ...
  // ─── MAIN RENDER ────────────────────────────────────────────────────────────
  return (
    <div style={{fontFamily:"system-ui,-apple-system,'Segoe UI',Roboto,sans-serif",background:"linear-gradient(160deg,#2e2b2a 0%,#252525 55%,#1c1c1c 100%)",minHeight:"100vh",color:"#E2E0E1",fontSize:14}}>

      {/* ── HEADER ── */}
      <div style={{background:"linear-gradient(180deg,#38322e 0%,#2b2b2b 100%)",borderBottom:"1px solid #454545",padding:"10px 16px",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{width:32,height:32,borderRadius:9,background:"linear-gradient(135deg,#4a3f38 0%,#333333 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:4,boxShadow:"0 2px 8px rgba(0,0,0,0.4)"}}><DamfieldLogo size={24}/></div>
          <div>
            <div style={{fontSize:14,fontWeight:800}}>Damfield</div>
            <div style={{display:"flex",alignItems:"center",gap:5}}>
              <div style={{fontSize:9,color:"#7c7876",textTransform:"uppercase",letterSpacing:1}}>Alquileres</div>
              <span style={{fontSize:8,color:"#BA9F82",fontWeight:700,background:"#BA9F8220",borderRadius:4,padding:"1px 5px",letterSpacing:0.3}}>v{APP_VERSION}</span>
            </div>
          </div>
        </div>
        ...
        {/* Nav */}
        <div style={{display:"flex",background:"#333333",borderRadius:8,padding:3,border:"1px solid #454545",gap:2}}>
          {[
            ["reservas","📅 Reservas"],
            ["clientes","👥 Clientes"],
            ...
            <button key={v} onClick={()=>setView(v)} style={{
              ...
              background:view===v?"linear-gradient(135deg,#d0b08a,#BA9F82)":"transparent",
              color:view===v?"#2b2b2b":"#7c7876",
            }}>{l}</button>
          ))}
        </div>
        ...
        {canEdit&&(
          <button onClick={()=>openNew(new Date(),18,"futbol_11")} style={{
            background:"linear-gradient(135deg,#d0b08a 0%,#BA9F82 60%,#9e8464 100%)",color:"#2b2b2b",border:"none",
      ...
      {view==="reservas"&&reservaView==="semana"&&(
        <div style={{overflowX:"auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",borderBottom:"1px solid #454545",background:"#2b2b2b",position:"sticky",top:0,zIndex:10,flexWrap:"wrap"}}>
            {/* Sub-view selector */}
            <div style={{display:"flex",background:"#333333",borderRadius:6,padding:2,border:"1px solid #454545",gap:1,marginRight:4}}>
              {[["dia","Día"],["semana","Semana"],["mes","Mes"],["lista","Lista"]].map(([v,l])=>(
                <button key={v} onClick={()=>setReservaView(v)} style={{padding:"3px 9px",borderRadius:5,border:"none",cursor:"pointer",fontSize:11,fontWeight:600,background:reservaView===v?"linear-gradient(135deg,#d0b08a,#BA9F82)":"transparent",color:reservaView===v?"#2b2b2b":"#7c7876"}}>{l}</button>
              ))}
            </div>
            <div style={{width:1,height:20,background:"#454545"}}/>
            ...
            <button onClick={()=>{const d=new Date(currentDate);d.setDate(d.getDate()+7);setCurrentDate(d);}} style={navBtn}>›</button>
            <span style={{fontSize:12,fontWeight:600,color:"#a7a3a0",textTransform:"capitalize"}}>{fmtMonthYear(weekDates[0])}</span>
            <div style={{width:1,height:20,background:"#454545",margin:"0 4px"}}/>
            {SPACE_GROUPS.map(({label,g})=>(
              <button key={g} onClick={()=>toggleGroup(g)} style={{
                ...
                borderColor:visibleGroups.includes(g)?groupColor(g):"#454545",
                background:visibleGroups.includes(g)?`${groupColor(g)}1a`:"transparent",
                color:visibleGroups.includes(g)?groupColor(g):"#7c7876",
              }}>{label}</button>
            ))}
          </div>
          {visibleSpaceIds.length===0?(
            <div style={{textAlign:"center",padding:60,color:"#7c7876"}}>Seleccioná al menos un grupo.</div>
          ):(
            <div style={{minWidth:700}}>
              <div style={{display:"grid",gridTemplateColumns:"80px repeat(7,1fr)",borderBottom:"1px solid #454545",background:"#2b2b2b",position:"sticky",top:37,zIndex:9}}>
                <div/>
                {weekDates.map((date,i)=>{
                  const today=dateKey(date)===dateKey(new Date());
                  return(
                    <div key={i} style={{padding:"6px 4px",textAlign:"center",borderLeft:"1px solid #454545"}}>
                      <div style={{fontSize:9,color:"#7c7876",textTransform:"uppercase"}}>{DAYS[i]}</div>
                      <div style={{fontSize:16,fontWeight:700,color:today?"#2b2b2b":"#E2E0E1",background:today?"linear-gradient(135deg,#d0b08a,#BA9F82)":"transparent",borderRadius:6,padding:today?"2px 6px":"0"}}>{date.getDate()}</div>
                    </div>
                  );
                })}
              </div>
                ...
                return(
                  <div key={spaceId}>
                    <div style={{display:"grid",gridTemplateColumns:"80px repeat(7,1fr)",background:`${sp.color}18`,borderTop:`2px solid ${sp.color}55`}}>
                      <div style={{padding:"5px 8px",display:"flex",flexDirection:"column",alignItems:"flex-start",justifyContent:"center",gap:2,borderRight:"1px solid #454545",borderLeft:`4px solid ${sp.color}`}}>
                      ...
                      {weekDates.map((_,di)=><div key={di} style={{borderLeft:"1px solid #454545"}}/>)}
                    </div>
                          ...
                          {weekDates.map((date,di)=>{
                            ...
                            return(
                              <div key={di}
                                ...
                                style={{borderLeft:"1px solid #0f1219",background:isBlocked?"#3a3030":today?"#ffffff05":"transparent",cursor:isBlocked?'not-allowed':(isOcc&&!(isEscritorio&&cellBks.length<ESCRITORIO_MAX))||!canEdit?'default':'pointer',position:"relative",minHeight:44,transition:"background 0.1s"}}
                                onMouseEnter={e=>{if(!isOcc&&!isBlocked&&canEdit)e.currentTarget.style.background=`${sp.color}28`;}}
                                onMouseLeave={e=>{e.currentTarget.style.background=isBlocked?"#3a3030":today?"#ffffff05":"transparent";}}
                              >
                                {isBlocked&&!isOcc&&<div style={{position:"absolute",inset:2,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:9,color:"#5c5c5c",letterSpacing:0.3}}>bloqueado</span></div>}
                                ...
                                {cellBks.length>1?(
                                  ...
                                  const bg=isBlq?"#3f1a1a":bk.sinCargo?"#454545":`${sp.color}22`;
                                  const bdr=isBlq?"1px solid #7f1d1d":bk.sinCargo?"1px solid #454545":`1px solid ${sp.color}55`;
                                  const clr=isBlq?"#c96b5f":bk.sinCargo?"#7c7876":sp.color;
                                  return(
      ...
      {view==="reservas"&&reservaView==="dia"&&(()=>{
        ...
        return(
          <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 110px)",overflow:"hidden"}}>
            {/* Sticky nav bar */}
            <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",borderBottom:"1px solid #454545",background:"#2b2b2b",flexShrink:0,flexWrap:"wrap"}}>
              <div style={{display:"flex",background:"#333333",borderRadius:6,padding:2,border:"1px solid #454545",gap:1,marginRight:4}}>
                {[["dia","Día"],["semana","Semana"],["mes","Mes"],["lista","Lista"]].map(([v,l])=>(
                  <button key={v} onClick={()=>setReservaView(v)} style={{padding:"3px 9px",borderRadius:5,border:"none",cursor:"pointer",fontSize:11,fontWeight:600,background:reservaView===v?"linear-gradient(135deg,#d0b08a,#BA9F82)":"transparent",color:reservaView===v?"#2b2b2b":"#7c7876"}}>{l}</button>
                ))}
              ...
              <button onClick={()=>setCurrentDate(new Date())} style={{...navBtn,fontSize:11,padding:"3px 10px"}}>Hoy</button>
              <button onClick={()=>{const d=new Date(currentDate);d.setDate(d.getDate()+1);setCurrentDate(d);}} style={navBtn}>›</button>
              <span style={{fontSize:13,fontWeight:700,color:todayStr?"#BA9F82":"#a7a3a0",textTransform:"capitalize"}}>
                {dow}, {currentDate.toLocaleDateString("es-AR",{day:"numeric",month:"long",year:"numeric"})}
              </span>
              {todayStr&&<span style={{fontSize:10,color:"#BA9F82",fontWeight:700,background:"#BA9F8220",borderRadius:4,padding:"2px 6px"}}>Hoy</span>}
            </div>
            {/* Grid */}
            <div style={{flex:1,overflowX:"auto",overflowY:"hidden"}}>
              <div style={{display:"grid",gridTemplateColumns:`48px repeat(${allCols.length},1fr)`,gridTemplateRows:`28px repeat(${N},1fr)`,height:"100%",minWidth:600}}>
                {/* Corner */}
                <div style={{background:"#2b2b2b",borderRight:"1px solid #454545",borderBottom:"1px solid #454545"}}/>
                {/* Column headers */}
                {allCols.map(({sid,group},ci)=>{
                  const sp=SPACES[sid];
                  return(
                    <div key={sid} style={{background:"#2b2b2b",borderBottom:"1px solid #454545",borderRight:ci<allCols.length-1?"1px solid #454545":"none",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"2px 2px",gap:1}}>
                      <div style={{width:8,height:8,borderRadius:"50%",background:sp.color,flexShrink:0}}/>
                      <span style={{fontSize:9,fontWeight:700,color:"#a7a3a0",textTransform:"uppercase",letterSpacing:0.3,lineHeight:1.1,textAlign:"center"}}>{sp.short}</span>
                    </div>
                  );
                })}
        ...
        return(
          <div style={{overflowX:"auto"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",borderBottom:"1px solid #454545",background:"#2b2b2b",position:"sticky",top:0,zIndex:10,flexWrap:"wrap"}}>
              ...
              <button onClick={()=>setCurrentDate(new Date())} style={{...navBtn,fontSize:11,padding:"3px 10px"}}>Hoy</button>
              <button onClick={()=>{const d=new Date(currentDate);d.setMonth(d.getMonth()+1);setCurrentDate(d);}} style={navBtn}>›</button>
              <span style={{fontSize:13,fontWeight:700,color:"#a7a3a0",textTransform:"capitalize"}}>{currentDate.toLocaleDateString("es-AR",{month:"long",year:"numeric"})}</span>
            </div>
            <div style={{minWidth:700,padding:"0 8px 16px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:2,marginTop:8}}>
                {["Lun","Mar","Mié","Jue","Vie","Sáb","Dom"].map(d=>(
                  <div key={d} style={{textAlign:"center",fontSize:10,fontWeight:700,color:"#7c7876",textTransform:"uppercase",padding:"4px 0"}}>{d}</div>
                ))}
              </div>
              {/* Grid */}
                ...
                {grid.map((date,i)=>{
                  ...
                  return(
                    <div key={i} onClick={()=>{setCurrentDate(date);setView("calendar");}}
                      style={{background:isToday?"#BA9F820d":"#333333",border:`1px solid ${isToday?"#BA9F8244":"#454545"}`,borderRadius:8,minHeight:100,padding:"5px 6px",cursor:"pointer",opacity:isCurrentMonth?1:0.4,transition:"background 0.12s"}}
                      onMouseEnter={e=>e.currentTarget.style.background=isToday?"#BA9F8215":"#3d3d3d"}
                      onMouseLeave={e=>e.currentTarget.style.background=isToday?"#BA9F820d":"#333333"}
                    >
                      {/* Número del día */}
                      <div style={{display:"flex",justifyContent:"flex-end",marginBottom:4}}>
                        <div style={{width:22,height:22,borderRadius:"50%",background:isToday?"#BA9F82":"transparent",display:"flex",alignItems:"center",justifyContent:"center"}}>
                          <span style={{fontSize:12,fontWeight:isToday?800:600,color:isToday?"#2b2b2b":isCurrentMonth?"#E2E0E1":"#7c7876"}}>{date.getDate()}</span>
                        </div>
                      </div>
                      {/* Bookings */}
                      ...
                      {extra>0&&<div style={{fontSize:9,color:"#7c7876",marginTop:2}}>+{extra} más</div>}
                    </div>
                  );
                })}
              </div>
      ...
      {view==="reservas"&&reservaView==="lista"&&(()=>{
        ...
        <div>
          {/* Nav lista */}
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",borderBottom:"1px solid #454545",background:"#2b2b2b",position:"sticky",top:0,zIndex:10,flexWrap:"wrap"}}>
            <div style={{display:"flex",background:"#333333",borderRadius:6,padding:2,border:"1px solid #454545",gap:1}}>
              {[["dia","Día"],["semana","Semana"],["mes","Mes"],["lista","Lista"]].map(([v,l])=>(
                <button key={v} onClick={()=>setReservaView(v)} style={{padding:"3px 9px",borderRadius:5,border:"none",cursor:"pointer",fontSize:11,fontWeight:600,background:reservaView===v?"linear-gradient(135deg,#d0b08a,#BA9F82)":"transparent",color:reservaView===v?"#2b2b2b":"#7c7876"}}>{l}</button>
              ))}
            </div>
          ...
          <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
            ...
            {/* Time range */}
            <div style={{display:"flex",background:"#2b2b2b",borderRadius:8,padding:3,border:"1px solid #454545",gap:2}}>
              {[["proximos","Próximos"],["todos","Todos"],["pasados","Pasados"],["semana","Semana"],["mes","Mes"],["año","Año"]].map(([v,l])=>(
                <button key={v} onClick={()=>{setListTimeRange(v);if(v!==listTimeRange)setListTimeDate(new Date());}} style={{padding:"4px 10px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:listTimeRange===v?"#BA9F82":"transparent",color:listTimeRange===v?"#2b2b2b":"#7c7876"}}>{l}</button>
              ))}
            </div>
            {/* Date nav para semana/mes/año */}
            {["semana","mes","año"].includes(listTimeRange)&&(
              <div style={{display:"flex",alignItems:"center",gap:5,background:"#2b2b2b",borderRadius:8,padding:"3px 6px",border:"1px solid #454545"}}>
                ...
                <span style={{fontSize:11,fontWeight:700,color:"#a7a3a0",minWidth:140,textAlign:"center"}}>
            ...
            {/* Bloqueos toggle */}
            <button onClick={()=>setListShowBloqueos(p=>!p)} style={{padding:"4px 12px",borderRadius:20,border:`1px solid ${listShowBloqueos?"#c96b5f":"#454545"}`,background:listShowBloqueos?"#c96b5f18":"transparent",color:listShowBloqueos?"#c96b5f":"#7c7876",cursor:"pointer",fontSize:11,fontWeight:600}}>
              🚫 {listShowBloqueos?"Ocultar bloqueos":"Mostrar bloqueos"}
            </button>
            <span style={{fontSize:11,color:"#7c7876",marginLeft:"auto"}}>{listItems.length} resultado{listItems.length!==1?"s":""}</span>
          </div>
          {listItems.map(bk=>{
            const sp=SPACES[bk.space];
            ...
            const isBlq=bk.isBloqueo;
            return(
              <div key={bk.id} onClick={()=>openEdit(bk)} style={{background:isBlq?"#1a0d0d":"#2b2b2b",border:`1px solid ${isBlq?"#7f1d1d33":sp.color+"2a"}`,borderLeft:`4px solid ${isBlq?"#c96b5f":sp.color}`,borderRadius:10,padding:"10px 14px",marginBottom:7,cursor:"pointer",display:"flex",gap:14,alignItems:"center",transition:"background 0.12s"}}
                onMouseEnter={e=>e.currentTarget.style.background=isBlq?"#220d0d":"#151c2c"}
                onMouseLeave={e=>e.currentTarget.style.background=isBlq?"#1a0d0d":"#2b2b2b"}
              >
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:11,color:"#7c7876"}}>{sp.label}{!isBlq&&bk.clientOrg?` · ${bk.clientOrg}`:""}</div>
                ...
                <div style={{textAlign:"right"}}>
                  ...
                  bk.sinCargo?<span style={{fontSize:10,color:"#7c7876"}}>Sin cargo</span>:(
                    <>
          ...
          {listItems.length===0&&<div style={{textAlign:"center",padding:60,color:"#7c7876"}}>No hay registros para este filtro.</div>}
          </div>
        </div>
        );
      })()}
      ...
      {view==="clientes"&&(()=>{
        ...
        <div style={{padding:16,maxWidth:860,margin:"0 auto"}}>
          ...
          <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
            ...
            <div style={{display:"flex",background:"#2b2b2b",borderRadius:8,padding:3,border:"1px solid #454545",gap:2}}>
              <button onClick={()=>setClientFilter("all")} style={{padding:"4px 10px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:clientFilter==="all"?"#BA9F82":"transparent",color:clientFilter==="all"?"#2b2b2b":"#7c7876"}}>Todos</button>
              <button onClick={()=>setClientFilter("incompletos")} style={{padding:"4px 10px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:clientFilter==="incompletos"?"#f59e0b":"transparent",color:clientFilter==="incompletos"?"#000":"#7c7876"}}>⚠ Incompletos</button>
            </div>
          ...
          {shown.length===0&&<div style={{textAlign:"center",padding:60,color:"#7c7876"}}><div style={{fontSize:32,marginBottom:10}}>👥</div><div>Sin clientes para este filtro.</div></div>}
          {shown.map(c=>{
            ...
            return(
              <div key={c.id} onClick={()=>canEdit&&openClientModal(c)} style={{background:"#2b2b2b",border:`1px solid ${incomplete?"#f59e0b33":"#454545"}`,borderLeft:`4px solid ${incomplete?"#f59e0b":"#454545"}`,borderRadius:12,padding:"14px 16px",marginBottom:8,cursor:canEdit?"pointer":"default",transition:"background 0.12s"}}
              ...
              >
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                  <div style={{flex:1}}>
                    ...
                    {c.org&&<div style={{fontSize:11,color:"#7c7876"}}>{c.org}</div>}
                  </div>
                  <div style={{textAlign:"right",marginLeft:12}}>
                    <div style={{fontSize:13,fontWeight:700,color:"#22c55e"}}>{fmtMoney(total)}</div>
                    <div style={{fontSize:10,color:"#7c7876"}}>{hist.length} alquiler{hist.length!==1?"es":""}</div>
                  </div>
                </div>
                <div style={{display:"flex",gap:14,flexWrap:"wrap"}}>
                  {c.phone?<span style={{fontSize:11,color:"#7c7876"}}>📱 {c.phone}</span>:<span style={{fontSize:11,color:"#f59e0b55"}}>📱 sin tel.</span>}
                  {c.email?<span style={{fontSize:11,color:"#7c7876"}}>✉ {c.email}</span>:<span style={{fontSize:11,color:"#f59e0b55"}}>✉ sin email</span>}
                </div>
                {hist.length>0&&<div style={{marginTop:8,display:"flex",gap:5,flexWrap:"wrap"}}>{hist.slice(-5).map(bk=><span key={bk.id} style={{fontSize:9,background:"#454545",borderRadius:6,padding:"2px 7px",color:"#7c7876"}}>{new Date(bk.date+"T12:00").toLocaleDateString("es-AR",{day:"2-digit",month:"2-digit"})} · {SPACES[bk.space]?.short}</span>)}{hist.length>5&&<span style={{fontSize:9,color:"#7c7876"}}>+{hist.length-5} más</span>}</div>}
              </div>
            );
          })}
        </div>
      ...
      {view==="finanzas"&&canFinance&&(
        <div style={{padding:16,maxWidth:900,margin:"0 auto"}}>
          <div style={{display:"flex",gap:8,marginBottom:18,flexWrap:"wrap",alignItems:"center"}}>
            {/* Tipo de período */}
            <div style={{display:"flex",background:"#2b2b2b",borderRadius:8,padding:3,border:"1px solid #454545"}}>
              {[["semana","Semana"],["mes","Mes"],["año","Año"]].map(([v,l])=>(
                <button key={v} onClick={()=>{setFinPeriod(v);setFinDate(new Date());}} style={{padding:"5px 14px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:finPeriod===v?"#BA9F82":"transparent",color:finPeriod===v?"#2b2b2b":"#7c7876"}}>{l}</button>
              ))}
            </div>
            {/* Navegación */}
            <div style={{display:"flex",alignItems:"center",gap:6,background:"#2b2b2b",borderRadius:8,padding:"3px 6px",border:"1px solid #454545"}}>
              ...
              <span style={{fontSize:12,fontWeight:700,color:"#a7a3a0",minWidth:160,textAlign:"center"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:20}}>
            {[
              {label:"Total facturado",val:fmtMoney(finData.total),color:"#E2E0E1",sub:`${finData.count} alquiler${finData.count!==1?"es":""}`},
              ...
              <div key={c.label} style={{background:"#2b2b2b",border:"1px solid #454545",borderRadius:12,padding:"18px 20px"}}>
                <div style={{fontSize:10,color:"#7c7876",textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>{c.label}</div>
                <div style={{fontSize:24,fontWeight:800,color:c.color}}>{c.val}</div>
                {c.sub&&<div style={{fontSize:10,color:"#7c7876",marginTop:3}}>{c.sub}</div>}
              </div>
            ))}
          </div>
          ...
          {finData.cobrado>0&&(
            <div style={{marginBottom:20}}>
              <div style={{fontSize:10,color:"#7c7876",textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>Cobrado por forma de pago</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8}}>
                {finData.FORMAS.map(([f,l])=>finData.byForma[f]>0&&(
                  <div key={f} style={{background:"#2b2b2b",border:"1px solid #454545",borderRadius:10,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontSize:12,color:"#a7a3a0"}}>{l}</span>
                ...
                {finData.FORMAS.every(([f])=>!finData.byForma[f])&&(
                  <div style={{gridColumn:"1/-1",fontSize:11,color:"#7c7876",padding:"8px 0"}}>Sin pagos registrados con forma de pago en este período.</div>
                )}
              </div>
            </div>
          )}
          ...
          {Object.keys(finData.bySpace).length>0&&(
            <div>
              <div style={{fontSize:10,color:"#7c7876",textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>Detalle por espacio</div>
              {Object.entries(finData.bySpace).map(([spId,d])=>{
                const sp=SPACES[spId], pct=d.total>0?Math.round((d.cobrado/d.total)*100):0;
                return(
                  <div key={spId} style={{background:"#2b2b2b",border:`1px solid ${sp.color}2a`,borderLeft:`4px solid ${sp.color}`,borderRadius:10,padding:"12px 16px",marginBottom:8}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                      <div><span style={{fontWeight:700,fontSize:13}}>{sp.label}</span><span style={{fontSize:10,color:"#7c7876",marginLeft:8}}>{d.count} alquiler{d.count!==1?"es":""}</span></div>
                      <div><span style={{fontWeight:800,color:"#22c55e",fontSize:14}}>{fmtMoney(d.cobrado)}</span><span style={{color:"#7c7876",fontSize:12}}> / {fmtMoney(d.total)}</span></div>
                    </div>
                    <div style={{background:"#454545",borderRadius:4,height:5}}><div style={{width:`${pct}%`,background:sp.color,height:"100%",borderRadius:4}}/></div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              ...
              <FG label="Contraseña">
                <div style={{position:"relative"}}>
                  ...
                  <button type="button" onClick={()=>setShowUserPwd(p=>!p)} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#7c7876",fontSize:16,padding:2}}>
                    {showUserPwd?"🙈":"👁"}
                  </button>
                </div>
              </FG>
          ...
          {users.map(u=>(
            <div key={u.id} style={{background:"#2b2b2b",border:"1px solid #454545",borderRadius:10,padding:"12px 16px",marginBottom:8,display:"flex",alignItems:"center",gap:12}}>
              ...
              <div style={{fontSize:10,padding:"3px 10px",borderRadius:20,fontWeight:700,
                ...
                color:u.role==="admin"?"#f59e0b":u.role==="vendedor"?"#BA9F82":"#60a5fa"}}>
        ...
        return(
          <>
            ...
            <div style={{position:"fixed",left,top,zIndex:99,background:"#161b2a",border:`1px solid ${sp.color}55`,borderRadius:12,minWidth:260,maxWidth:320,boxShadow:"0 8px 32px #00000088",overflow:"hidden"}}>
              {/* Header */}
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 12px",borderBottom:"1px solid #454545"}}>
                {selected?(
                ...
                <button onClick={()=>setCellPicker(null)} style={{background:"none",border:"none",color:"#7c7876",cursor:"pointer",fontSize:14,lineHeight:1,padding:"0 2px"}}>✕</button>
              </div>
                ...
                return(
                  <div style={{padding:"12px 14px"}}>
                    ...
                    <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
                      ...
                      {bk.sinCargo&&<span style={{fontSize:10,background:"#454545",borderRadius:6,padding:"3px 8px",color:"#7c7876"}}>Sin cargo</span>}
                      {isPast(bk)&&bk.asistio!==null&&(
                        <span style={{fontSize:10,background:"#454545",borderRadius:6,padding:"3px 8px",color:bk.asistio?"#22c55e":"#c96b5f",fontWeight:700}}>
                          {bk.asistio?"✓ Asistió":"✗ No asistió"}
                        </span>
                      )}
                    </div>
                    {/* Notas */}
                    {bk.notes&&<div style={{fontSize:10,color:"#7c7876",background:"#454545",borderRadius:6,padding:"6px 8px",marginBottom:10,lineHeight:1.5}}>📝 {bk.notes}</div>}
                    {/* Botón editar */}
                  ...
                  {bks.map(bk=>{
                    ...
                    return(
                      <div key={bk.id} onClick={()=>setCellPicker(p=>({...p,selected:bk}))}
                        style={{padding:"7px 10px",borderRadius:8,marginBottom:5,cursor:"pointer",background:"#454545",border:"1px solid #454545",transition:"background 0.1s"}}
                        onMouseEnter={e=>e.currentTarget.style.background="#253048"}
                        onMouseLeave={e=>e.currentTarget.style.background="#454545"}>
      ...
      {modal&&(isMobile?(
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(24,22,21,0.68)",backdropFilter:"blur(3px)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:100,padding:16,paddingTop:40,overflowY:"auto"}}
          onClick={e=>e.target===e.currentTarget&&setModal(false)}>
          <div style={{background:"linear-gradient(160deg,#333333 0%,#2b2b2b 100%)",border:"1px solid #454545",borderRadius:8,width:"100%",maxWidth:520,boxShadow:"0 20px 50px -12px rgba(0,0,0,0.7)",padding:"18px 22px",marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <h2 style={{margin:0,fontSize:17,fontWeight:800}}>{editing?"Editar alquiler":"Nuevo alquiler"}</h2>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                {editing&&<button onClick={()=>setAuditModal(editing)} style={{background:"none",border:"1px solid #454545",color:"#7c7876",cursor:"pointer",fontSize:11,borderRadius:6,padding:"3px 8px"}}>🕑 Historial</button>}
                <button onClick={()=>setModal(false)} style={{background:"none",border:"none",color:"#7c7876",cursor:"pointer",fontSize:22}}>×</button>
              </div>
            </div>

            {/* Audit info */}
            {editing&&editing.createdBy&&(
              <div style={{background:"#333333",borderRadius:8,padding:"7px 12px",marginBottom:14,fontSize:10,color:"#7c7876",display:"flex",gap:16,flexWrap:"wrap"}}>
                <span>✏️ Creado por <b style={{color:"#7c7876"}}>{editing.createdBy}</b> · {fmtDateTime(editing.createdAt)}</span>
                {editing.updatedBy&&editing.updatedAt!==editing.createdAt&&<span>🔄 Modificado por <b style={{color:"#7c7876"}}>{editing.updatedBy}</b> · {fmtDateTime(editing.updatedAt)}</span>}
              </div>
            )}
            ...
            <div style={{display:"flex",gap:6,marginBottom:14}}>
              {[{v:false,l:"📅 Alquiler"},{v:true,l:"🚫 Bloquear espacio"}].map(({v,l})=>(
                <button key={String(v)} onClick={()=>{setForm(f=>({...f,isBloqueo:v}));setModalTab('reserva');}} style={{flex:1,padding:"8px",borderRadius:8,border:`2px solid ${form.isBloqueo===v?(v?"#c96b5f":"#BA9F82"):"#454545"}`,background:form.isBloqueo===v?(v?"#c96b5f18":"#BA9F8218"):"transparent",color:form.isBloqueo===v?(v?"#c96b5f":"#BA9F82"):"#7c7876",cursor:"pointer",fontWeight:700,fontSize:12}}>{l}</button>
              ))}
            </div>

            {/* Tabs — solo en modo Alquiler */}
            {!form.isBloqueo&&(
              <div style={{display:"flex",gap:0,background:"transparent",borderBottom:"1px solid #454545",marginBottom:16}}>
                {[{v:"reserva",l:"📅 Reserva"},{v:"cliente",l:"👤 Cliente"},{v:"cobro",l:"💰 Cobro"}].map(({v,l})=>(
                  <button key={v} onClick={()=>setModalTab(v)} style={{flex:1,padding:"9px 0",border:"none",borderBottom:modalTab===v?"2px solid #BA9F82":"2px solid transparent",cursor:"pointer",fontSize:13,fontWeight:modalTab===v?700:500,background:"transparent",color:modalTab===v?"#E2E0E1":"#7c7876",transition:"color 0.12s,border-color 0.12s"}}>{l}</button>
                ))}
              </div>
            )}
            ...
            {form.isBloqueo?(
              <FG label="Motivo del bloqueo *">
                <input value={form.bloqueoMotivo||""} onChange={e=>setForm(f=>({...f,bloqueoMotivo:e.target.value}))} placeholder="Ej: Academia, Torneo nocturno…" style={{...inpSt,borderColor:!(form.bloqueoMotivo||"").trim()?"#c96b5f66":"#454545"}}/>
                {!(form.bloqueoMotivo||"").trim()&&<div style={{fontSize:10,color:"#c96b5f",marginTop:3}}>Requerido</div>}
              </FG>
            ):(
              <>
            {modalTab==='reserva'&&<>
            <label style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,cursor:"pointer",background:form.sinCargo?"#BA9F820d":"#333333",padding:"9px 12px",borderRadius:10,border:`1px solid ${form.sinCargo?"#BA9F8244":"#454545"}`}}>
              <input type="checkbox" checked={form.sinCargo} onChange={e=>setForm(f=>({...f,sinCargo:e.target.checked}))} style={{width:15,height:15,accentColor:"#BA9F82"}}/>
              <div><div style={{fontSize:13,fontWeight:700,color:form.sinCargo?"#BA9F82":"#a7a3a0"}}>🎁 Sin cargo</div><div style={{fontSize:10,color:"#7c7876"}}>Cortesía, uso interno…</div></div>
            </label>
            {form.sinCargo&&(
              <FG label="Motivo del sin cargo *">
                <input value={form.sinCargoMotivo||""} onChange={e=>setForm(f=>({...f,sinCargoMotivo:e.target.value}))} placeholder="Ej: cortesía sponsor, uso interno…" style={{...inpSt,borderColor:!(form.sinCargoMotivo||"").trim()?"#c96b5f66":"#454545"}}/>
                {!(form.sinCargoMotivo||"").trim()&&<div style={{fontSize:10,color:"#c96b5f",marginTop:3}}>Requerido para guardar</div>}
              </FG>
            )}
            </>}
              </>
            ...
            {/* Recurrencia */}
            <div style={{background:"#333333",borderRadius:10,padding:"12px 14px",marginBottom:12,border:"1px solid #454545"}}>
              <div style={{fontSize:11,fontWeight:700,color:"#a7a3a0",marginBottom:10}}>🔁 Repetición semanal</div>
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                {[["none","No repetir"],["count","N semanas"],["until","Hasta fecha"]].map(([v,l])=>(
                  <button key={v} onClick={()=>setForm(f=>({...f,recurrence:v}))} style={{padding:"4px 12px",borderRadius:20,border:"1px solid",fontSize:11,cursor:"pointer",borderColor:form.recurrence===v?"#BA9F82":"#454545",background:form.recurrence===v?"#BA9F8222":"transparent",color:form.recurrence===v?"#BA9F82":"#7c7876"}}>{l}</button>
                ))}
              </div>
              {form.recurrence==="count"&&<div style={{marginTop:10,display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:12,color:"#7c7876"}}>Semanas:</span><input type="number" min={1} max={52} value={form.recurrenceCount} onChange={e=>setForm(f=>({...f,recurrenceCount:+e.target.value}))} style={{...inpSt,width:60}}/><span style={{fontSize:11,color:"#7c7876"}}>{form.recurrenceCount} alquileres</span></div>}
              {form.recurrence==="until"&&<div style={{marginTop:10,display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:12,color:"#7c7876"}}>Hasta:</span><input type="date" value={form.recurrenceUntil} onChange={e=>setForm(f=>({...f,recurrenceUntil:e.target.value}))} style={{...inpSt,width:"auto"}}/></div>}
            </div>
            </>)}
            ...
            {!form.isBloqueo&&modalTab==='cliente'&&(
              <>
              {form.sinCargo?(
                <div style={{textAlign:"center",padding:"40px 0",color:"#7c7876",fontSize:13}}>🎁 Sin cargo — no requiere cliente.</div>
              ):(
              <>
                <div style={{fontSize:9,color:"#7c7876",textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>Cliente</div>
                <div style={{marginBottom:10}}>
                  {form.clientName?(
                    <div style={{display:"flex",alignItems:"center",gap:8,background:"#333333",borderRadius:8,padding:"8px 12px",border:"1px solid #454545"}}>
                      <div style={{flex:1}}><div style={{fontSize:13,fontWeight:700}}>{form.clientName}</div>{form.clientOrg&&<div style={{fontSize:11,color:"#7c7876"}}>{form.clientOrg}</div>}</div>
                      <button onClick={()=>setForm(f=>({...f,clientId:"",clientName:"",clientPhone:"",clientEmail:"",clientOrg:""}))} style={{background:"none",border:"none",color:"#7c7876",cursor:"pointer",fontSize:16}}>×</button>
                    </div>
                  ):(
                    <button onClick={()=>setClientPicker(true)} style={{width:"100%",padding:"9px 12px",borderRadius:8,border:"1px dashed #454545",background:"transparent",color:"#7c7876",cursor:"pointer",fontSize:12,textAlign:"left"}}>👥 Elegir cliente existente…</button>
                  )}
                </div>
                {!form.clientId&&(
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            ...
            {!form.isBloqueo&&modalTab==='cobro'&&(
              <>
              {form.sinCargo?(
                <div style={{textAlign:"center",padding:"40px 0",color:"#7c7876",fontSize:13}}>🎁 Sin cargo — no hay cobro registrado.</div>
              ):(
              <>
                <div style={{fontSize:9,color:"#7c7876",textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>Pago</div>
                ...
                {/* Descuento */}
                <div style={{background:"#333333",borderRadius:10,padding:"10px 12px",marginBottom:10,border:"1px solid #454545"}}>
                  <div style={{fontSize:10,color:"#7c7876",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>🏷 Descuento</div>
                  <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
                    {[["none","Sin descuento"],["pct","% Porcentaje"],["monto","$ Fijo"]].map(([v,l])=>(
                      <button key={v} onClick={()=>setForm(f=>({...f,descuentoTipo:v,descuentoValor:""}))} style={{padding:"4px 10px",borderRadius:20,border:"1px solid",fontSize:11,cursor:"pointer",borderColor:form.descuentoTipo===v?"#f59e0b":"#454545",background:form.descuentoTipo===v?"#f59e0b22":"transparent",color:form.descuentoTipo===v?"#f59e0b":"#7c7876",fontWeight:600}}>{l}</button>
                    ))}
                  </div>
                ...
                {Number(form.totalAmount)>0&&Number(form.señaAmount)>0&&(
                  <div style={{background:"#333333",borderRadius:8,padding:"8px 12px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontSize:11,color:"#7c7876"}}>Saldo a cobrar el día del partido</span>
                    <span style={{fontSize:14,fontWeight:800,color:"#BA9F82"}}>{fmtMoney(getNetAmount(form)-Number(form.señaAmount))}</span>
                  </div>
                )}
                ...
                {(()=>{
                  ...
                  return(
                    <>
                      {pagos.length>0&&(
                        <div style={{marginBottom:10}}>
                          <div style={{fontSize:9,color:"#7c7876",textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>Pagos registrados</div>
                          {pagos.map(p=>(
                            <div key={p.id} style={{display:"flex",alignItems:"center",gap:8,background:"#333333",borderRadius:7,padding:"6px 10px",marginBottom:4}}>
                              <span style={{fontSize:10,color:"#7c7876",minWidth:34}}>{p.fecha?new Date(p.fecha+"T12:00").toLocaleDateString("es-AR",{day:"2-digit",month:"2-digit"}):"—"}</span>
                              <span style={{fontSize:13,fontWeight:700,color:"#22c55e",flex:1}}>{fmtMoney(p.monto)}</span>
                              {p.forma&&<span style={{fontSize:9,padding:"2px 7px",borderRadius:20,background:"#454545",color:"#7c7876",fontWeight:600}}>{{efectivo:"💵 Efectivo",transferencia:"🏦 Transfer.",debito:"💳 Débito",credito:"💳 Crédito",cuentacorriente:"📒 Cta.Cte."}[p.forma]||p.forma}</span>}
                              {p.nota&&<span style={{fontSize:10,color:"#7c7876"}}>{p.nota}</span>}
                          ...
                          <div style={{display:"flex",justifyContent:"space-between",padding:"7px 10px",borderTop:"1px solid #454545",marginTop:2}}>
                            <span style={{fontSize:11,color:"#7c7876"}}>Total cobrado</span>
                      ...
                      {/* Agregar pago */}
                      {canEdit&&(
                        <div style={{background:"#333333",borderRadius:10,padding:"12px",marginBottom:10,border:"1px dashed #454545"}}>
                          <div style={{fontSize:9,color:"#BA9F82",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>+ Registrar pago recibido</div>
            ...
            {editing&&!form.sinCargo&&isPast({date:form.date,endHour:form.endHour})&&(
              <>
                <div style={{borderTop:"1px solid #454545",margin:"10px 0"}}/>
                <div style={{fontSize:9,color:"#7c7876",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Asistencia</div>
              ...
              return(
                <div style={{display:"flex",gap:8,marginTop:16}}>
                  ...
                  {!form.isBloqueo&&tabIdx>0&&(
                    <button onClick={()=>setModalTab(MODAL_TABS[tabIdx-1])} style={{padding:"8px 14px",borderRadius:8,border:"1px solid #454545",background:"transparent",color:"#a7a3a0",cursor:"pointer",fontSize:12}}>← Anterior</button>
                  )}
                  {canEdit&&(isLast?(
                    <button onClick={save} disabled={disabled} style={{padding:"8px 20px",borderRadius:8,border:"none",fontSize:12,fontWeight:700,cursor:"pointer",background:disabled?"#454545":"#BA9F82",color:disabled?"#7c7876":"#2b2b2b"}}>{editing?"Guardar":form.isBloqueo?"Bloquear":"Crear alquiler"}</button>
                  ):(
                    <button onClick={()=>setModalTab(MODAL_TABS[tabIdx+1])} style={{padding:"8px 20px",borderRadius:8,border:"none",fontSize:12,fontWeight:700,cursor:"pointer",background:"linear-gradient(135deg,#d0b08a 0%,#BA9F82 60%,#9e8464 100%)",color:"#2b2b2b"}}>Siguiente →</button>
                  ))}
                </div>
              );
            })()}
      ...
      {auditModal&&(
        <div style={{position:"fixed",inset:0,background:"#000000d0",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}
          ...
          <div style={{background:"#2b2b2b",border:"1px solid #454545",borderRadius:14,width:"100%",maxWidth:420,maxHeight:"70vh",overflowY:"auto",padding:20}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <h3 style={{margin:0,fontSize:15,fontWeight:700}}>🕑 Historial de cambios</h3>
              <button onClick={()=>setAuditModal(null)} style={{background:"none",border:"none",color:"#7c7876",cursor:"pointer",fontSize:20}}>×</button>
            </div>
            {(auditModal.history||[]).length===0&&<div style={{color:"#7c7876",fontSize:12}}>Sin historial.</div>}
            {[...(auditModal.history||[])].reverse().map((h,i)=>(
              <div key={i} style={{background:"#333333",borderRadius:8,padding:"8px 12px",marginBottom:6}}>
                <div style={{fontSize:12,fontWeight:700,color:"#E2E0E1"}}>{h.action}</div>
                <div style={{fontSize:11,color:"#7c7876"}}>por <b>{h.by}</b> · {fmtDateTime(h.at)}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {clientPicker&&(
        <div style={{position:"fixed",inset:0,background:"#000000d0",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}
          ...
          <div style={{background:"#2b2b2b",border:"1px solid #454545",borderRadius:14,width:"100%",maxWidth:400,maxHeight:"70vh",overflowY:"auto",padding:18}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <h3 style={{margin:0,fontSize:15,fontWeight:700}}>Elegir cliente</h3>
              <button onClick={()=>setClientPicker(false)} style={{background:"none",border:"none",color:"#7c7876",cursor:"pointer",fontSize:20}}>×</button>
            </div>
            ...
            {clients.filter(c=>c.name.toLowerCase().includes(clientSearch.toLowerCase())||(c.org||"").toLowerCase().includes(clientSearch.toLowerCase())).map(c=>(
              <div key={c.id} onClick={()=>pickClient(c)} style={{padding:"10px 12px",borderRadius:8,border:"1px solid #454545",marginBottom:6,cursor:"pointer",transition:"background 0.12s"}}
                onMouseEnter={e=>e.currentTarget.style.background="#1a2030"}
                onMouseLeave={e=>e.currentTarget.style.background="transparent"}
              >
                <div style={{fontSize:13,fontWeight:700}}>{c.name}</div>
                <div style={{fontSize:11,color:"#7c7876"}}>{c.org||""}{c.phone?` · ${c.phone}`:""}</div>
              </div>
            ))}
            {clients.length===0&&<div style={{textAlign:"center",padding:30,color:"#7c7876",fontSize:12}}>No hay clientes aún.</div>}
          </div>
        </div>
      )}
      ...
      {clientModal&&(
        <div style={{position:"fixed",inset:0,background:"#000000d0",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}
          ...
          <div style={{background:"#2b2b2b",border:"1px solid #454545",borderRadius:14,width:"100%",maxWidth:420,padding:22,maxHeight:"85vh",overflowY:"auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <h3 style={{margin:0,fontSize:16,fontWeight:700}}>{editingClient?"Editar cliente":"Nuevo cliente"}</h3>
              <button onClick={()=>setClientModal(false)} style={{background:"none",border:"none",color:"#7c7876",cursor:"pointer",fontSize:20}}>×</button>
            </div>
            ...
            {editingClient&&(()=>{
              ...
              return hist.length>0?(
                <div style={{marginTop:12,borderTop:"1px solid #454545",paddingTop:12}}>
      ...
      {confirmModal&&(
        <div style={{position:'fixed',inset:0,background:'#000000d0',display:'flex',alignItems:'center',justifyContent:'center',zIndex:500,padding:16}}
          ...
          <div style={{background:'#2b2b2b',border:'1px solid #454545',borderRadius:14,width:'100%',maxWidth:380,padding:24}}>
            <div style={{fontSize:16,fontWeight:800,marginBottom:8,color:confirmModal.danger?'#c96b5f':'#E2E0E1'}}>{confirmModal.title}</div>
            <div style={{fontSize:13,color:'#a7a3a0',marginBottom:20,lineHeight:1.5}}>{confirmModal.body}</div>
            <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
              <button onClick={()=>setConfirmModal(null)}
                style={{padding:'8px 16px',borderRadius:8,border:'1px solid #454545',background:'transparent',color:'#a7a3a0',cursor:'pointer',fontSize:13}}>
                Cancelar
              </button>
...
function FG({label,children}){
  return(
    <div style={{marginBottom:10}}>
      <label style={{display:"block",fontSize:11,color:"#a7a3a0",textTransform:"uppercase",letterSpacing:"0.04em",marginBottom:4,fontWeight:500}}>{label}</label>
      {children}
    </div>
  );
}

const navBtn ={background:"#333333",border:"1px solid #454545",color:"#a7a3a0",borderRadius:6,padding:"3px 10px",cursor:"pointer",fontSize:15,transition:"background 0.12s"};
const inpSt  ={width:"100%",background:"#333333",border:"1px solid #454545",borderRadius:8,padding:"10px 12px",color:"#E2E0E1",fontSize:14,outline:"none",boxSizing:"border-box",transition:"border-color 0.12s"};
const selSt  ={background:"#333333",border:"1px solid #454545",borderRadius:8,padding:"10px 12px",color:"#E2E0E1",fontSize:14,outline:"none"};
const lblSt  ={display:"block",fontSize:11,color:"#a7a3a0",marginBottom:5,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.04em"};
