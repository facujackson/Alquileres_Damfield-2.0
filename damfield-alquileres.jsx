...
export default function App() {
  ...
  useEffect(()=>{
    async function load(){
      ...
      if(cl) setClients(cl);
      // Siempre partir de DEFAULT_USERS - luego agregar extras creados por admin
      const extras = (us && Array.isArray(us)) ? us.filter(u => !DEFAULT_USERS.find(d => d.id === u.id)) : [];
  ...
  // Solo persistir usuarios EXTRA (creados por admin), nunca los defaults
  useEffect(()=>{
    if(loaded) {
      const extras = users.filter(u => !DEFAULT_USERS.find(d => d.id === u.id));
      dbSet("users", extras);
    }
  },[users,loaded]);
  ...
  if(!loaded) return (
    <div style={{minHeight:"100vh",background:"#0b0e17",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}>
      <div style={{width:40,height:40,borderRadius:12,background:"linear-gradient(135deg,#22c55e,#3b82f6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>🏟</div>
      <div style={{color:"#475569",fontSize:13}}>Cargando sistema…</div>
    </div>
  );
